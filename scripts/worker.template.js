...
async function tableHasColumn(db, table, column) {
  ...
  const result = await db
    .prepare(`SELECT 1 FROM pragma_table_info('${safeTable}') WHERE name=? LIMIT 1`)
    .bind(column)
    .first();
  return Boolean(result);
}

async function prepareRuntimeMigration(db, migration) {
  const statements = [];
  for (const statement of splitSqlStatements(migration.sql)) {
    const trimmed = statement.trim();
    if (!trimmed || /^PRAGMA\s+user_version\s*=/i.test(trimmed)) continue;
    ...
    if (addColumn) {
      ...
      const column = addColumn[2].replace(/["`]/g, "");
      if (await tableHasColumn(db, table, column)) continue;
    }
...
async function ensureDatabase(env) {
  if (!env.DB) {
    throw new Error("D1 数据库绑定 DB 不存在，请检查 Cloudflare Pages 的 D1 Binding（变量名必须为 DB）。");
  }

  let promise = databaseReady.get(env);
  if (!promise) {
    promise = (async () => {
      // This table is created through the D1 Worker binding, so Git/Pages builds
      // do not need a Cloudflare API token just to initialize the database.
      ...
      const appliedRows = await env.DB
        .prepare("SELECT id FROM _stnav_migrations")
        .all();
      const applied = new Set((appliedRows.results ?? []).map((row) => String(row.id)));

      for (const migration of RUNTIME_MIGRATIONS) {
        if (applied.has(migration.id)) continue;

        const statements = await prepareRuntimeMigration(env.DB, migration);
        statements.push(
          env.DB.prepare("INSERT OR IGNORE INTO _stnav_migrations(id) VALUES(?)").bind(migration.id)
        );
        ...
        try {
          ...
          const alreadyApplied = await env.DB
            .prepare("SELECT 1 FROM _stnav_migrations WHERE id=? LIMIT 1")
            .bind(migration.id)
            .first();
      ...
      if (missing.length) {
        throw new Error(`D1 数据库初始化后仍缺少数据表: ${missing.join(", ")}`);
      }
    })();
